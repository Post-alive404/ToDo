...
import com.dden.todo.client.ToDoRestClient;
...
public class TodoClientApplication {
	...
	public CommandLineRunner process(ToDoRestClient client){
		return args -> {
			...
			assert toDos != null;
			toDos.forEach(toDo -> log.info("findAll " + toDo.toString()));

			ToDo newToDo = client.upsert(new ToDo("Drink plenty of Water daily!"));
			assert newToDo != null;
			log.info("upsert " + newToDo.toString());

			ToDo toDo = client.findById(newToDo.getId());
			assert toDos != null;
			log.info("findById " + toDo.toString());

			ToDo completed = client.setCompleted(newToDo.getId());
			assert completed.isCompleted();
			log.info("setCompleted " + completed.toString());


			toDos.forEach(toDoi -> log.info("findAll " + toDoi.toString()));
